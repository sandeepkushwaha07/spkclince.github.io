...
function fifu_get_strings_settings() {
    ...
    $fifu['auto']['important'] = function () {
        _e("Depending on the post title and the filters applied, the search engine might return an image that's not very relevant, or even no image at all. Using the 'Source filter' usually works better than the 'License filter,' since not all images are categorized by license.", FIFU_SLUG);
    };
    ...
    $fifu['video']['desc'] = function () {
        _e("FIFU supports videos from YouTube, Vimeo, Twitter, 9GAG, Cloudinary, Tumblr, Publitio, JW Player, VideoPress, Sprout, Odysee, Rumble, Dailymotion, Cloudflare Stream, Bunny Stream, Amazon, BitChute, Brighteon, and Google Drive. It also supports external and local video files.", FIFU_SLUG);
    };
    ...
    $fifu['autoplay']['desc'] = function () {
        _e("Video autoplay (available for YouTube, Vimeo, Odysee, Rumble, and local videos). Requires \"Mute\".", FIFU_SLUG);
    };

    // loop
    $fifu['loop']['desc'] = function () {
        _e("Looping video playback (available for YouTube, Vimeo, and local videos).", FIFU_SLUG);
    };

    // mute
    $fifu['mute']['desc'] = function () {
        _e("Start videos without audio (available for YouTube, Vimeo, and local videos).", FIFU_SLUG);
    };
...
function fifu_get_strings_help() {
    ...
    $fifu['desc']['keywords'] = function () {
        return __("Loads a list of images from Unsplash. Choose the image that is most suitable.", FIFU_SLUG);
    };
